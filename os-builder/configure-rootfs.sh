...
[[ -z "\$DISPLAY" ]] && (( EUID )) && [ -x /usr/local/bin/portalwallet-session ] && exec /usr/local/bin/portalwallet-session
[[ -z "\$DISPLAY" ]] && (( EUID )) && exit
EOF
...
install -D -m 0755 "$FILES/portalwallet-session" "$ROOTFS/usr/local/bin/portalwallet-session"
...
exec /usr/bin/portalwallet >/tmp/portalwallet.log 2>&1
EOF
...
# /usr/lib/portalwallet and stays root-owned, so the setuid chrome-sandbox keeps 4755 root.
