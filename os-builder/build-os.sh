...
HASHLOCK="$REPO/os-builder/lock/rootfs-package-hash-lock.txt"
...
mmdebstrap \
	...
	--customize-hook="copy-in $PORTALWALLET_DEB /tmp" \
	--customize-hook="chroot \"\$1\" apt-get install -y --no-install-recommends /tmp/$(basename "$PORTALWALLET_DEB")" \
	--customize-hook="chroot \"\$1\" rm -f /tmp/$(basename "$PORTALWALLET_DEB")" \
	trixie \
	"$ROOTFS" \
...
# from rootfs-package-version-lock.list and already verified above, and boot is
