#!/usr/bin/env bash
# Build the complete portalwallet disk image, start to finish, inside the container.
#
#   mmdebstrap  -> Debian rootfs from the pinned live+snapshot sources
#   portalwallet.deb   -> the reproducible Electron app package (built separately by
...
ROOT_PARTUUID=9021c0de-0000-4000-8000-000000000003
...
PORTALWALLET_DEB="$REPO/build/portalwallet_${VERSION}_amd64.deb"
if [ ! -f "$PORTALWALLET_DEB" ]; then
	echo "ERROR: $PORTALWALLET_DEB not found — run 'make deb-package' first" >&2
	echo "       (found instead: $(ls "$REPO"/build/*.deb 2>/dev/null | tr '\n' ' ' || echo none))" >&2
	exit 1
fi
echo "Using app package: $(basename "$PORTALWALLET_DEB")"
...
_other="$(ls "$REPO"/build/portalwallet_*_amd64.deb 2>/dev/null | grep -v "^$PORTALWALLET_DEB$" || true)"
...
#
# portalwallet.deb ships no maintainer scripts but does ship a .desktop file,
# which activates the desktop-file-utils trigger during install — a real
# script, so /proc is mounted here rather than assumed unnecessary.
...
cp "$PORTALWALLET_DEB" "$ROOTFS/tmp/"
chroot "$ROOTFS" apt-get install -y --no-install-recommends "/tmp/$(basename "$PORTALWALLET_DEB")"
rm -f "$ROOTFS/tmp/$(basename "$PORTALWALLET_DEB")"
...
APP_PKG="$(dpkg-deb -f "$PORTALWALLET_DEB" Package)=$(dpkg-deb -f "$PORTALWALLET_DEB" Version)"
...
echo portalwallet > "$ROOTFS/etc/hostname"
...
# tree.
#
# Two boot chains, selected by PORTALWALLET_BOOT (`make BOOT=...`).
...
#                    hash if not.
BOOT_MODE="${PORTALWALLET_BOOT:-grub}"
...
case "$BOOT_MODE" in
grub)
	if [ "${PORTALWALLET_BOOTSIGN:-no}" != "no" ]; then
		echo "ERROR: BOOTSIGN=${PORTALWALLET_BOOTSIGN} is meaningless with BOOT=grub — nothing of ours" >&2
	...
	;;
uki)
	# PORTALWALLET_BOOTSIGN defaults to "no" (matching `BOOTSIGN ?= no` in the Makefile): builds are
	...
	SB_KEY=/secureboot/portalwallet.key
	SB_CRT=/secureboot/portalwallet.crt
	...
	BOOTSIGN_MODE="${PORTALWALLET_BOOTSIGN:-no}"
	...
	if [ "$BOOTSIGN_MODE" = "no" ]; then
		...
		echo "       Expected portalwallet.key + portalwallet.crt (mounted from os-builder/secureboot/)." >&2
		echo "       Run 'make secureboot-key' for a dev pair, or build with BOOTSIGN=no." >&2
		exit 1
	fi
	SIGN_ARGS=()
	if [ -r "$SB_KEY" ] && [ -r "$SB_CRT" ]; then
		echo "==> boot: signed UKI (enroll EFI/portalwallet.der once per machine via MokManager)"
	...
	if [ -r "$SB_CRT" ]; then
		openssl x509 -in "$SB_CRT" -outform DER -out "$WORK/portalwallet.der"
	else
		: > "$WORK/portalwallet.der"   # placeholder; genimage requires the file to exist
	fi
...
# ---------------------------------------------------------------------------
mkdir -p "$OUT"
IMG="$OUT/portalwallet-v${VERSION}.img"
mv "$WORK/portalwallet.img" "$IMG"
