...
# out if it is set with BOOT=grub, where nothing of ours is signed. UNSIGNED BY DEFAULT so that any two builds of
# the same commit produce a byte-identical image: an Authenticode signature
# embeds a signing time, so a signed image can never be byte-reproducible.
...
DEB_BUILDER_IMAGE := portalwallet-deb-builder
OS_BUILDER_IMAGE  := portalwallet-os-builder
DEB := build/portalwallet_$(BUILD_VERSION)_amd64.deb
RAW := build/portalwallet-v$(BUILD_VERSION).img
# gzip -n: no filename/mtime in the member header, so identical .img bytes
# always produce identical .img.gz bytes (both are attested).
IMG_GZ := $(RAW).gz
...
distclean: clean
	...
	docker image rm -f portalwallet-deb-builder portalwallet-os-builder 2>/dev/null || true
	docker buildx prune --force --builder reproducible-builder 2>/dev/null || true
	# Remove the .tar/.state builder-image cache (see DOCKER_CACHE_DIR above) so
	# the next `make build` re-runs `docker buildx build` from scratch too.
...
build-deb-check:
	...
	cp $(DEB) /tmp/portalwallet-deb-1.deb
	$(MAKE) --no-print-directory deb-package
	cp $(DEB) /tmp/portalwallet-deb-2.deb
	sha256sum /tmp/portalwallet-deb-1.deb /tmp/portalwallet-deb-2.deb
	@if cmp -s /tmp/portalwallet-deb-1.deb /tmp/portalwallet-deb-2.deb; then \
...
os-image: boot-check deb-package os-builder
	...
	docker run --rm $(DOCKER_CPUSET) \
		...
		-e HOST_UID=$(shell id -u) \
		-e HOST_GID=$(shell id -g) \
		-e PORTALWALLET_BOOTSIGN=$(BOOTSIGN) \
		-e PORTALWALLET_BOOT=$(BOOT) \
		$(OS_BUILDER_IMAGE) \
...
secureboot-key:
	...
	@if [ -e os-builder/secureboot/portalwallet.key ]; then \
		echo "REFUSING: os-builder/secureboot/portalwallet.key already exists."; \
		exit 1; \
	fi
	...
	docker run --rm \
		...
		bash -c 'apt-get -qq update && apt-get -qq install -y openssl >/dev/null && \
			...
			-subj "/CN=portalwallet Secure Boot/" \
			-keyout /secureboot/portalwallet.key -out /secureboot/portalwallet.crt 2>/dev/null && \
			chmod 600 /secureboot/portalwallet.key && \
			chown $${HOST_UID}:$${HOST_GID} /secureboot/portalwallet.key /secureboot/portalwallet.crt'
